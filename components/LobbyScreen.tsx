  ...
  if (currentTab !== GameState.LOBBY) {
    return (
      <div className="flex flex-col h-screen bg-om-black">
        <div className="pt-safe px-4 pb-2 glass-nav">
          <div className="flex justify-between items-center h-12">
            <button onClick={() => onTabChange(GameState.LOBBY)} className="p-2 -ml-2 text-white/30 hover:text-white/60 transition-colors">
        ...
        <div className="glass-nav fixed bottom-0 left-0 w-full flex justify-around items-center h-20 px-2 z-50">
  ...
  return (
    <div className="flex flex-col h-screen bg-om-black overflow-hidden relative">
      ...
      <div className="pt-safe px-6 pb-6 bg-gradient-to-b from-om-burgundy/20 to-transparent">
        <div className="flex justify-between items-start pt-4 mb-6">
          <div className="flex flex-col">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] text-om-gold font-bold tracking-[0.3em] uppercase glass-badge px-2 py-0.5">{rank}</span>
            </div>
            <h1 className="text-2xl font-cinzel font-black tracking-tight text-om-cream">{userName}</h1>
          </div>
          <button onClick={() => onTabChange(GameState.WHEEL)} className="group relative w-12 h-12 rounded-full bg-gold-gradient p-0.5 shadow-gold-glow active:scale-95 transition-all glass-shimmer">
        ...
        <div className="glass-panel-intense p-5 relative overflow-hidden group">
      ...
      {/* News Ticker with Glass Effect */}
      <div className="w-full glass-panel border-x-0 rounded-none py-2 overflow-hidden whitespace-nowrap">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 scrollbar-hide pb-32">
        <div className="mb-6">
           <div className="flex items-center gap-4 mb-4">
              ...
              <span className="text-[10px] font-cinzel font-bold text-om-gold tracking-[0.4em] uppercase glass-badge px-3 py-1">{t.salon}</span>
           ...
           <div className="grid grid-cols-2 gap-3">
              {games.map((game) => (
                <button
                  ...
                  className={`glass-card relative overflow-hidden aspect-[1.1/1] p-4 flex flex-col justify-between group active:scale-[0.98] transition-all`}
                >
                  <div className="absolute inset-0 bg-gradient-to-br ${game.color} opacity-20 group-hover:opacity-30 transition-opacity"></div>
                  <div className="relative text-om-gold/40 group-hover:text-om-gold transition-colors">{game.icon}</div>
        ...
        {/* Jackpot Banner with Glass Effect */}
        <div className="glass-panel-intense p-6 mb-6 relative overflow-hidden">
